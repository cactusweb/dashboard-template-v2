...
export class LicenseService {
  ...
  getLicense(update: boolean = false): Observable<License> {
    if (
      (update || !this.license) &&
      !this.loading &&
      this.lastFetch + 10 * 1000 < Date.now()
    ) {
      this.fetchLicense()
        .pipe(take(1))
    ...
    // @ts-ignore
    return this.$license.asObservable().pipe(filter((u) => !!u));
  }

  public fetchLicense(): Observable<License> {
    this.loading = true;
    return this.http.request(Requests['getLicense']).pipe(
      map((l: License) => {
        return {
          ...l,
  ...
  public resetLicense(): Observable<undefined> {
    return this.http.request(Requests['resetLicense']).pipe(
      tap(() => {
  ...
  public unbindLicense(): Observable<undefined> {
    return this.http.request(Requests['unbindLicense']).pipe(
  ...
  public bindLicense(data: { key: string }): Observable<License> {
    return this.http.request(Requests['bindLicense'], data).pipe(
      map((l: License) => {
        return {
          ...l,
          expires_in: l.expires_in * 1000,
  ...
  public cancelPayment(): Observable<any> {
    return this.http.request(Requests['stopSub']).pipe(
      take(1),
      tap(() => {
  ...
  public getReferral(): Observable<ReferralPrize[]> {
    if (this.referralPrizes)
      return new BehaviorSubject<ReferralPrize[]>(
        this.referralPrizes.prizes
      ).asObservable();

    return this.http.request(Requests['getReferralPrizes']).pipe(
      tap((d) => (this.referralPrizes = d)),
      map((d) => d.prizes)
    );
  }

  public getAdditionalActivationPlans(): Observable<
    AdditionalActivationPlan[]
  > {
    if (this.additionalActivationPlans)
      return of(this.additionalActivationPlans);

    return this.http.request(Requests['getAdditionalActivationPlans']).pipe(
      map((d) => d.plans),
      tap((d) => (this.additionalActivationPlans = d))
    );
  }

  public renewLicense(renewDuration: number = 1) {
    if (!this.license) return;

    this.license = {
      ...this.license,
      expires_in: this.license.expires_in
        ? new Date(this.license.expires_in).setMonth(
            new Date(this.license.expires_in).getMonth() + renewDuration
          )
